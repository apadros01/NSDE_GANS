...
#         torch.nn.Linear(mlp_size, mlp_size),torch.nn.ReLU(),torch.nn.Dropout(p=0.35)]
...
#         torch.nn.init.kaiming_uniform_(self._model[0].weight,mode='fan_in', nonlinearity='relu'),
...
class MLP(torch.nn.Module):
    def __init__(self, in_size, out_size, mlp_size, num_layers, tanh, disc = False):
        ...
        if disc:
            model = [torch.nn.Linear(in_size, mlp_size),torch.nn.ReLU(),torch.nn.BatchNorm1d(mlp_size),
            torch.nn.Linear(mlp_size, mlp_size),torch.nn.ReLU(),torch.nn.Dropout(p=0.2)]
            for _ in range(num_layers - 1):
                model.append(torch.nn.Linear(mlp_size, mlp_size))
                # model.append(torch.nn.BatchNorm1d(mlp_size))
                model.append(torch.nn.ReLU())
                # model.append(torch.nn.BatchNorm1d(mlp_size))

        else:
            model = [torch.nn.Linear(in_size, mlp_size),torch.nn.ReLU(),
            torch.nn.Linear(mlp_size, mlp_size),torch.nn.ReLU()]
        ...
        self._model = torch.nn.Sequential(*model)
        torch.nn.init.kaiming_uniform_(self._model[0].weight,mode='fan_in', nonlinearity='relu'),
        # torch.nn.init.kaiming_normal_(self._model[0].weight, mode='fan_out', nonlinearity='relu')
        # torch.nn.init.ones_(self._model[0].weight)
...
class Generator(torch.nn.Module):
    ...
    def forward(self, ts, batch_size):
        ...
        ###################
        xs = torchsde.sdeint_adjoint(self._func, x0, ts, method='reversible_heun', dt=1,
...
class Discriminator(torch.nn.Module):
    def __init__(self, data_size, hidden_size, mlp_size, num_layers):
        ...
        self._initial = MLP(data_size, hidden_size, mlp_size, num_layers, tanh=False, disc=True)
#         self._func = DiscriminatorFunc(data_size, hidden_size, mlp_size, num_layers)
        self._readout = torch.nn.Linear(mlp_size, 1)
...
#         Y0 = Y.evaluate(Y.interval[0])
        ...
        ys_coeffs = ys_coeffs.reshape((ys_coeffs.shape[0],ys_coeffs.shape[1]))
